...
if (supabaseUrl && supabaseKey && !supabaseUrl.includes('seu-projeto')) {
  ...
  console.warn('\x1b[33m%s\x1b[0m', '⚠️  Aviso: Variáveis SUPABASE_URL e SUPABASE_KEY não configuradas no ambiente. Usando armazenamento temporário.');
}

// ── FALLBACK EM MEMÓRIA (caso as variáveis ainda não estejam na Vercel) ────────
let _nextModeloId = 4;
let _nextMovId    = 11;
let _mockModelos = [
...
async function deleteModelo(id) {
  ...
  if (!supabase) {
    const idx = _mockModelos.findIndex(m => m.id === numId);
    if (idx !== -1) {
      _mockModelos.splice(idx, 1);
    }
    _mockMovs = _mockMovs.filter(m => m.modeloId !== numId);
    return true;
  }
...
async function getMovimentacoes(modeloId) {
  const numModeloId = modeloId ? Number(modeloId) : null;
  if (!supabase) {
    const list = numModeloId
      ? _mockMovs.filter(m => m.modeloId === numModeloId)
      : _mockMovs;
  ...
  if (numModeloId) {
    query = query.eq('modelo_id', numModeloId);
  }
...
async function createMovimentacao({ modeloId, tipo, qty, setor, resp, data, obs }) {
  const numModeloId = Number(modeloId);
  const numQty = Number(qty);

  if (!supabase) {
    const mov = { 
      id: _nextMovId++, 
      modeloId: numModeloId, 
      tipo, 
      qty: numQty, 
      setor, 
      resp, 
...
async function deleteMovimentacao(id) {
  const numId = Number(id);
  if (!supabase) {
    const idx = _mockMovs.findIndex(m => m.id === numId);
    if (idx !== -1) {
